...
void Channel::addClient(Client &client, std::string password)
{
	...
	if (find == _member.end())
	{
		...
		//broadcastMessage(RPL_JOINING(client->getNick(), _channel), client.getFd());
		std::cout << RPL_JOINING(client.getNick(), _channel) << std::endl;
...
void	Channel::topic(Client& member, std::string topic)
{
	...
	if (_topic.empty())
	{
		broadcastMessage(member, RPL_NOTOPIC(_channel), member.getFd()); // No topic is set
		//std::cout << RED << "Channel " << _channel << " dosen't have a topic." << RESET << std::endl;
	}
	else
	{
		broadcastMessage(member, RPL_TOPIC(_channel, _topic), member.getFd()); // Channel :topic
